...
class ReceiveMessage:
    ...
    def __filterJSONResponseForMessagesOfUser(self, jsonResponse, userName: str):
        ...
        for data in jsonResponse:
            if (data["content"] in Payloads.list()) and data["author"][
                "username"
            ] == userName:
    ...
    def receiveBotMessages(
        ...
        user_message_list = self.__filterJSONResponseForMessagesOfUser(
            json_data, botUserName
        )
        user_message_list = self.__filterMessageListForAge(
            user_message_list, maxSecondsOld
        )
        user_message_list = self.__filterMessageListForUsedFields(
            user_message_list, self.usedFieldsInMessages
        )
        print(
            f"retrieved messages of bot: {botUserName} that are max. {maxSecondsOld} seconds old"
        )
        return user_message_list

    def get_last_bot_message_id_in_last_150_seconds(self, channel_string):
        messageID = 0
        messages = self.receiveBotMessages(
            channel_string, token_secret, 5, "EnesEY", 150
        )
