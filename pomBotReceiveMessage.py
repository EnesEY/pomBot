...
class PomBotReceive:
    ...
    def __getBotMessages(self, max_seconds_old, messages_searched_limit, channelID):
        bot_messages = {}
        print(
            f"searching in the last {messages_searched_limit} messages of the channel for messages from bot that are max. {max_seconds_old} seconds old"
        )
        json_data = self.__getAllMessages(
            channelID, AuthenticationTokens.AUTH_TOKEN_1.value, messages_searched_limit
        )
        # filter for bot messages that are max_seconds_old
        for data in json_data:
            if (data["content"] in Payloads.list()) and data["author"][
                "username"
            ] == "EnesEY":  # check if message is one from out bot
