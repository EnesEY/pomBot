...
class PomBotSend:
    ...
    def startEmojiToTest(self):
        self.__sendPomMessage(
            ChannelIDs.CHANNEL_ID_BOT_TEST.value,
            Payloads.PAYLOAD_POM_START_EMOJI.value,
            AuthenticationTokens.AUTH_TOKEN_1.value,
        )

    def doneEmojiToTest(self):
        self.__sendPomMessage(
            ChannelIDs.CHANNEL_ID_BOT_TEST.value,
            Payloads.PAYLOAD_POM_DONE_EMOJI.value,
            AuthenticationTokens.AUTH_TOKEN_1.value,
        )

    def startEmojiToMogli(self):
        self.__sendPomMessage(
            ChannelIDs.CHANNEL_ID_MOWGLI_DM_GROUP.value,
            Payloads.PAYLOAD_POM_START_EMOJI.value,
            AuthenticationTokens.AUTH_TOKEN_1.value,
        )
